...
const QuestionForm = ({ setShowForm, setQuestions, questions, notifySuccess, isEditing, editingQuestion }) => {
    const notifyError = (errMsg = "Couldn't create question") => toast.error(errMsg);
    const { token } = useToken();

    const onCreate = async (values) => {
        await PostRequest("/questions/", values, token)
            .then((response) => {
                if (response == null) {
                    notifyError()
    ...
    const onUpdate = async (values) => {
        await UpdateRequest("/questions/" + editingQuestion.id, values, token)
            .then((response) => {
                if (response == null) {
                    notifyError()
                } else if (response.status !== 201) {
                    ...
                    notifySuccess()
                    let objIndex = questions.findIndex((obj => obj.id === response.data[0].id));
                    ...
                    <Form className="flex flex-col w-96">
                        ...
                        <div className="mb-4">
                            <Field
                                ...
                                component="textarea"
                            />
                            {touched.question && <p className="text-red-500 text-sm">{errors.question}</p>}
                        </div>
